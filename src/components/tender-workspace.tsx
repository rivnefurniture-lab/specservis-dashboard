...
export function TenderWorkspace({ viewerId }: { viewerId: string }) {
  ...
  const saveDraft = async () => {
    ...
    if (draft.participationDecision === "skip" && !draft.decisionReason.trim()) { setError("Для рішення «Не беремо участь» вкажіть коротку причину."); return; }
    ...
    await patchItem(selected, patch);
  };
  ...
  const closeCard = () => { setSelectedId(null); setDraft(null); };
  ...
  return <div className={styles.app}>
    ...
    {selected && draft ? createPortal(<div className={styles.backdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeCard(); }}><aside className={styles.drawer} role="dialog" aria-modal="true" aria-label={`Картка ${selected.tenderId}`}><header><div><span>{selected.workbookTracked ? "З EXCEL · " : "З PROZORRO · "}{selected.tenderId}</span><h2>{selected.title}</h2><p>{selected.buyerName} · {selected.buyerEdrpou || "ЄДРПОУ не вказано"}</p></div><button type="button" onClick={closeCard} aria-label="Закрити"><X size={20} /></button></header><div className={styles.drawerBody}>
      ...
      <section className={styles.step}><header><b>2</b><div><h3>Чи беремо участь?</h3><p>Оберіть рішення. Для відмови причина обов’язкова.</p></div></header><div className={styles.decisionButtons}>{Object.entries(decisionLabels).map(([value, label]) => <button type="button" key={value} className={draft.participationDecision === value ? styles.activeDecision : ""} onClick={() => { const nextDecision = value as TenderParticipationDecision; setDraft({ ...draft, participationDecision: nextDecision, workflowStatus: nextDecision === "skip" ? "closed" : nextDecision === "participate" && ["new", "review"].includes(draft.workflowStatus) ? "preparing" : draft.workflowStatus }); }}>{label}</button>)}</div><label className={styles.textField}><span>{draft.participationDecision === "skip" ? "Причина відмови *" : "Обґрунтування рішення"}</span><textarea value={draft.decisionReason} onChange={(event) => setDraft({ ...draft, decisionReason: event.target.value })} placeholder="ТЗ, ціна, наявність, регіон, строки або інша конкретна причина" /></label></section>
    ...
    </div><footer><span>Версія {selected.version} · оновлено {dateTime(selected.updatedAt)}</span><button type="button" onClick={() => void saveDraft()} disabled={savingId === selected.id || (!manager && selected.assignedAccountId !== viewerId)}>{savingId === selected.id ? <LoaderCircle className={styles.spin} size={16} /> : <Check size={16} />}{!manager && selected.assignedAccountId !== viewerId ? "Спочатку візьміть собі" : "Зберегти"}</button></footer></aside></div>, document.body) : null}
  </div>;
}
