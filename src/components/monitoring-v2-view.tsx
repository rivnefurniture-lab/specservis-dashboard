...
import {
  ...
  Columns3, ExternalLink, Filter, RefreshCw, Save, Search, Settings2, SlidersHorizontal, X,
} from "lucide-react";
import type {
  MonitoringFacet,
  MonitoringCpvNode,
...
const directionMap: Record<string, string[]> = {
  "Капбудівництво": ["construction", "design"],
  "Кондиціонування": ["conditioning", "ventilation"],
  "Сервіс": ["Сервіс"],
};
...
function RowDetails({ row, canManage, onSaved }: { row: MonitoringV2Row; canManage: boolean; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    const response = await fetch("/api/monitoring-v2", {
      ...
      body: JSON.stringify({ procurementId: row.procurementId, lotId: row.lotId, directionId: row.directions.find((item) => item.primary)?.id ?? null, status, comment, suggestedRule }),
    });
    setSaving(false);
  ...
  return (
    <div className={styles.details}>
      ...
      {canManage ? <div className={styles.reviewForm}><select value={status} onChange={(event) => setStatus(event.target.value as MonitoringReviewStatus)}>{Object.entries(reviewLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Короткий коментар до рішення" /><input value={suggestedRule} onChange={(event) => setSuggestedRule(event.target.value)} placeholder={status === "not_relevant" ? "Фраза для виключення (необов'язково)" : "Термін для словника (необов'язково)"} /><button type="button" onClick={() => void save()} disabled={saving}>{saving ? <RefreshCw className={styles.spin} size={15} /> : <Check size={15} />}Зберегти</button>{error ? <small>{error}</small> : null}</div> : null}
    </div>
  );
}
    ...
    <div className={styles.page}>
      ...
      <details id="monitoring-filters" className={styles.filters}>
        ...
        <div className={styles.filterGrid}>
          ...
          <MultiSelect label="Рішення перевірки" values={filters.reviewStatuses ?? []} options={Object.entries(reviewLabels).map(([value, label]) => ({ value, label }))} onChange={(values) => update("reviewStatuses", values as MonitoringV2Filters["reviewStatuses"])} />
      ...
      {showRules && canManage ? <section className={styles.settingsPanel}><header><div><b>Правила відбору · версія {payload?.ruleVersion || "—"}</b><small>Нова зміна створює окрему версію; стара лишається в історії.</small></div><button type="button" onClick={() => setShowRules(false)}><X size={16} /></button></header>{payload?.ruleSuggestions.length ? <><header><div><b>Пропозиції з ручних перевірок</b><small>Повторювані рішення команди можна одразу додати до словника.</small></div></header><div className={styles.ruleList}>{payload.ruleSuggestions.map((suggestion) => <span key={`${suggestion.directionId}:${suggestion.kind}:${suggestion.value}`}><b>{suggestion.directionId}</b><em>{kindLabels[suggestion.kind]}</em><i>{suggestion.value} · {suggestion.occurrences}</i><button type="button" onClick={() => void publishSuggestion(suggestion)}>Додати</button></span>)}</div></> : null}<div className={styles.ruleList}>{payload?.rules.map((rule) => <span key={rule.id}><b>{rule.directionLabel}</b><em>{kindLabels[rule.kind]}</em><i>{rule.value}</i><button type="button" onClick={() => void setRuleActive(rule, !rule.active)}>{rule.active ? "Вимкнути" : "Увімкнути"}</button></span>)}</div><footer className={styles.ruleForm}><select value={ruleDraft.directionId} onChange={(event) => setRuleDraft((current) => ({ ...current, directionId: event.target.value }))}>{payload?.facets.directions.map((direction) => <option value={direction.value} key={direction.value}>{direction.label}</option>)}</select><select value={ruleDraft.kind} onChange={(event) => setRuleDraft((current) => ({ ...current, kind: event.target.value as MonitoringRuleEntry["kind"] }))}>{Object.entries(kindLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input value={ruleDraft.value} onChange={(event) => setRuleDraft((current) => ({ ...current, value: event.target.value }))} placeholder="Код, термін, бренд або виключення" />{ruleDraft.kind.startsWith("cpv_") ? <label><input type="checkbox" checked={ruleDraft.includeDescendants} onChange={(event) => setRuleDraft((current) => ({ ...current, includeDescendants: event.target.checked }))} />Дочірні коди</label> : null}<button type="button" onClick={() => void publishRule()} disabled={!ruleDraft.value.trim()}><Save size={15} />Опублікувати версію</button></footer></section> : null}
