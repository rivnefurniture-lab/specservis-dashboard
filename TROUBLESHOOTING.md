...
- **Причина:** серверний драйвер повертає поля дат як `Date`, а генератор Excel помилково викликав на них рядковий `.slice()`. Великий `LIMIT` також робив складну вибірку нестабільною.
- **Fix:** дати нормалізуються незалежно від runtime-типу; Excel збирається порціями по 200 рядків із безпечним паралелізмом по шість запитів, а довідники та службові зведення читаються лише для першої порції.
